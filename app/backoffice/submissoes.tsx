...
export default function SubmissoesBackoffice() {
  ...
  async function carregarSetting() {
    try {
      const { data } = await supabase
        ...
        .single();
      if (data) setAprovacaoAutomatica(data.valor === "true");
    } catch {
      // tabela pode não existir ainda — mantém default true
    }
  }
  ...
  async function carregar() {
    ...
    let query = supabase
      ...
      .select(`
        id, descricao_user, estado, criado_em, foto_url, quantidade, xp_atribuido, co2_atribuido, agua_atribuida,
  ...
  async function aprovar(sub: any) {
    ...
    const co2 = parseFloat(((sub.catalogo_acoes?.co2_estimado || 0) * (sub.quantidade || 1)).toFixed(3));
    const agua = parseFloat(((sub.catalogo_acoes?.agua_estimada || 0) * (sub.quantidade || 1)).toFixed(3));
      ...
      <View style={s.section}>
        ...
        {filtradas.map(sub => (
          <View key={sub.id} style={s.tableRow}>
            ...
            <Text style={[s.td, { flex: 2 }]}>{sub.catalogo_acoes?.titulo}</Text>
            <Text style={[s.td, { flex: 2, color: C.dim }]} numberOfLines={2}>{sub.descricao_user || "—"}</Text>
