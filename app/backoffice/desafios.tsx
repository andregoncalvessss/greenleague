...
const FORM_VAZIO = {
  ...
  impacto: "ambos" as "co2" | "agua" | "ambos",
  unidades_permitidas: [] as string[],
};
...
export default function DesafiosBackoffice() {
  ...
  async function guardar() {
    ...
    setLoading(true);
    // Unidade default = primeira permitida (ou a atual/"Unidades").
    const unidadeDefault = form.unidades_permitidas[0] || form.unidade_medida || "Unidades";
    const payload = {
      titulo: form.titulo.trim(),
      descricao: form.descricao.trim() || null,
      categoria_id: form.categoria_id || null,
      xp_base: parseInt(form.xp_base),
      // Zera a métrica que o desafio não contabiliza.
      co2_estimado: form.impacto === "agua" ? 0 : (parseFloat(form.co2_estimado) || 0),
      agua_estimada: form.impacto === "co2" ? 0 : (parseFloat(form.agua_estimada) || 0),
      unidade_medida: unidadeDefault,
      unidades_permitidas: form.unidades_permitidas.length > 0 ? form.unidades_permitidas : null,
  ...
  function iniciarEdicao(a: any) {
    setForm({
      ...
      impacto: a.impacto === "co2" || a.impacto === "agua" ? a.impacto : "ambos",
      unidades_permitidas: Array.isArray(a.unidades_permitidas) ? a.unidades_permitidas : [],
    });
    setEditando(true);
  }
  ...
  function toggleUnidade(u: string) {
    setForm(f => ({
      ...f,
      unidades_permitidas: f.unidades_permitidas.includes(u)
        ? f.unidades_permitidas.filter(x => x !== u)
        : [...f.unidades_permitidas, u],
    }));
  }
  ...
  useEffect(() => { setPagina(0); }, [pesquisa, filtroCat, filtroAtivo, vista]);
  const totalPaginas = Math.ceil(filtradas.length / DESAFIOS_PER_PAGE);
  const filtradasPagina = filtradas.slice(pagina * DESAFIOS_PER_PAGE, (pagina + 1) * DESAFIOS_PER_PAGE);
  ...
  return (
    <BackofficeLayout>
      ...
      <View style={s.section}>
        ...
        {/* Tipo de impacto */}
        <Text style={s.label}>Impacto Contabilizado</Text>
        <View style={[s.segment, { marginBottom: 12 }]}>
          {([
            { v: "co2", label: "Só CO₂", icon: "leaf-outline" },
            { v: "agua", label: "Só Água", icon: "water-outline" },
            { v: "ambos", label: "Ambos", icon: "git-compare-outline" },
          ] as const).map(op => (
            <TouchableOpacity
              key={op.v}
              style={[s.segmentBtn, form.impacto === op.v && s.segmentBtnActive]}
              onPress={() => setForm(f => ({ ...f, impacto: op.v }))}
            >
              <Ionicons name={op.icon as any} size={15} color={form.impacto === op.v ? (isDark ? "#000" : "#FFF") : C.muted} />
              <Text style={[s.segmentText, form.impacto === op.v && s.segmentTextActive]}>{op.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        {/* Unidades permitidas */}
        <Text style={s.label}>
          Unidades que o utilizador pode escolher
          <Text style={{ color: C.dim, fontWeight: "400" }}>  ·  {form.unidades_permitidas.length === 0 ? "todas permitidas" : `${form.unidades_permitidas.length} selecionada(s)`}</Text>
        </Text>
        <View style={s.unidadesWrap}>
          {UNIDADES.map(u => {
            const ativo = form.unidades_permitidas.includes(u);
            return (
              <TouchableOpacity
                key={u}
                style={[s.unidadeChip, ativo && { backgroundColor: C.green, borderColor: C.green }]}
                onPress={() => toggleUnidade(u)}
              >
                {ativo && <Ionicons name="checkmark" size={13} color={isDark ? "#000" : "#FFF"} />}
                <Text style={[s.unidadeChipText, ativo && { color: isDark ? "#000" : "#FFF" }]}>{u}</Text>
              </TouchableOpacity>
            );
          })}
        </View>
        <Text style={s.hintSmall}>Se não selecionares nenhuma, o utilizador poderá escolher qualquer unidade da lista.</Text>

        {form.tipo === "equipa" && (
          <View style={s.formRow}>
            <View style={{ flex: 1 }}>
              <Text style={s.label}>Meta Coletiva ({form.unidades_permitidas[0] || form.unidade_medida || "Unidades"}) *</Text>
...
function makeStyles(C: any) { return StyleSheet.create({
  ...
  hintSmall: { ...Type.footnote, color: C.dim, fontWeight: "400", marginTop: 8, marginBottom: 4 },
  unidadesWrap: { flexDirection: "row", flexWrap: "wrap", gap: 8, marginTop: 2 },
  unidadeChip: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: C.bg, borderWidth: 1, borderColor: C.border, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 7 },
  unidadeChipText: { color: C.muted, fontSize: 13, fontWeight: "600" },
