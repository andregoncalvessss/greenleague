...
export default function MissoesBackoffice() {
  ...
  async function alterarXPBonus(id: string, valor: string) {
    const xp = parseInt(valor) || 0;
    await supabase.from("missoes_semanais").update({ xp_bonus: xp }).eq("id", id);
  }
            ...
            <View key={m.id} style={[s.missaoCard, !m.ativa && s.missaoCardInativa]}>
              ...
              <View style={s.xpBonusGroup}>
                ...
                <TextInput
                  ...
                  defaultValue={String(m.xp_bonus || 0)}
                  keyboardType="numeric"
                  onEndEditing={(e) => alterarXPBonus(m.id, e.nativeEvent.text)}
                  selectTextOnFocus
                />
              </View>
