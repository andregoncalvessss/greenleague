  ...
  return (
    <BackofficeLayout>
      ...
      <View style={s.section}>
        ...
        <Pagination page={paginaEscola} totalPages={totalPaginasEscola} onChange={setPaginaEscola} C={C} />
      </View>
      ...
      <View style={s.section}>
        ...
        <Pagination page={paginaCurso} totalPages={totalPaginasCurso} onChange={setPaginaCurso} C={C} />
      </View>
...
function Pagination({ page, totalPages, onChange, C }: { page: number; totalPages: number; onChange: (p: number) => void; C: any }) {
  if (totalPages <= 1) return null;

  return (
    <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "flex-end", gap: 10, marginTop: 10 }}>
      <TouchableOpacity onPress={() => onChange(Math.max(0, page - 1))} disabled={page === 0} hitSlop={6}>
        <Ionicons name="chevron-back" size={13} color={page === 0 ? C.border : C.dim} />
      </TouchableOpacity>
      ...
      <TouchableOpacity onPress={() => onChange(Math.min(totalPages - 1, page + 1))} disabled={page >= totalPages - 1} hitSlop={6}>
        <Ionicons name="chevron-forward" size={13} color={page >= totalPages - 1 ? C.border : C.dim} />
      </TouchableOpacity>
    </View>
  );
}
